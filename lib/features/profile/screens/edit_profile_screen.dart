...
class _EditProfileScreenState extends State<EditProfileScreen> {
  ...
  final List<String> _studyStreamOptions = ['Computer Science', 'Mathematics', 'Physics', 'Literature', 'Business', 'Law', 'Medicine'];
  ...
  void initState() {
    ...
    _regNumberController = TextEditingController(text: profile['registrationNumber']);
    _parentEmailController = TextEditingController(text: profile['parentEmail']);
    ...
    if (existingClass != null && existingClass.isNotEmpty) {
      if (!_classroomOptions.contains(existingClass)) _classroomOptions.add(existingClass);
    ...
    if (existingStream != null && existingStream.isNotEmpty) {
      if (!_studyStreamOptions.contains(existingStream)) _studyStreamOptions.add(existingStream);
      _selectedStudyStream = existingStream;
    }
  }
  ...
  Future<void> _saveProfile() async {
    ...
    if (mounted) {
      ...
      if (!textSuccess) {
        CustomSnackBar.showError(context, 'Erreur : Le serveur a refusé les données texte (Regarde le terminal).');
      } else if (!photoSuccess) {
        CustomSnackBar.showError(context, 'Erreur : Impossible d\'envoyer l\'image (Regarde le terminal).');
      } else {
          ...
          child: Column(
            children: [
              ...
              Align(
                ...
                child: Text('Academic Details', style: TextStyle(fontSize: 18, fontWeight: FontWeight.bold, color: Theme.of(context).colorScheme.onSurface)),
              ),
              const SizedBox(height: 15),
              _buildTextField('Registration Number', _regNumberController, Icons.badge_outlined),
              const SizedBox(height: 15),
              ...
              _buildTextField('Parent Email', _parentEmailController, Icons.family_restroom, isEmail: true),
              const SizedBox(height: 30),
